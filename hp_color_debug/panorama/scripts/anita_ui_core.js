...
(function () {
  ...
  const HP_HERO_DEBUG_ENABLED = true;
  const HP_HERO_DEBUG_MAX_LINES = 2000;
  var _hpHeroDebugLines = 0;
  var _hpHeroDebugLastSig = "";
  var _hpHeroDebugLastDetectedHero = "";
  var _hpHeroDebugCapNotified = false;
  var _hpHeroDebugSigCounts = {};

  function hpHeroDebugShouldLogRepeat(count) {
    return count <= 2 || count === 5 || count === 10 || count === 20 ||
      count === 50 || count === 100 || count % 250 === 0;
  }

  function hpHeroDebugValue(value) {
  ...
  function hpHeroDebugPreset(preset) {
    if (!preset) return "none";
    return String(preset.id || preset.key || preset.name || "unknown") +
      "|name=" + String(preset.name || "") +
      "|source=" + String(preset.source || "") +
      "|mode=" + String(preset.heroMode || "") +
      "|heroes=" + hpHeroDebugValue(preset.heroes || []);
  }

  function hpHeroDebug(tag, fields) {
    if (!HP_HERO_DEBUG_ENABLED) return;
    var sig = String(tag || "event");
    if (fields) {
      for (var key in fields) {
        if (!Object.prototype.hasOwnProperty.call(fields, key)) continue;
        sig += " " + key + "=" + hpHeroDebugValue(fields[key]);
      }
    }
    var repeatCount = (_hpHeroDebugSigCounts[sig] || 0) + 1;
    _hpHeroDebugSigCounts[sig] = repeatCount;
    if (sig === _hpHeroDebugLastSig || !hpHeroDebugShouldLogRepeat(repeatCount)) return;
    if (_hpHeroDebugLines >= HP_HERO_DEBUG_MAX_LINES) {
      if (!_hpHeroDebugCapNotified) {
        _hpHeroDebugCapNotified = true;
        try { $.Msg("[HP_HERO_DEBUG] cap_reached lines=" + _hpHeroDebugLines + " max=" + HP_HERO_DEBUG_MAX_LINES); } catch (eCap) {}
      }
      return;
    }
    _hpHeroDebugLastSig = sig;
    _hpHeroDebugLines++;
    if (repeatCount > 1) sig += " repeat=" + repeatCount;
    try { $.Msg("[HP_HERO_DEBUG] " + sig); } catch (eDebug) {}
  }
  hpHeroDebug("boot", {
    source: "hp_color_debug",
    build: "hero_switch"
  });
  ...
  function hpHeroEvaluateLobbyStatus() {
    ...
    if (gameState >= 0) {
      ...
      hpHeroDebug("lobby_state", { gameState: gameState });
    ...
    if (hpHeroHasHideoutClass(hud) || hpHeroHasHideoutClass(root)) {
      if (_hpHeroMatchStateConfirmedActive) _hpMatchResetLastReason = "lobby";
      hpHeroDebug("lobby_hideout", { gameState: gameState });
      _hpHeroMatchStateConfirmedActive = false;
      return true;
    }
    ...
    if (!_hpHeroMatchStateConfirmedActive) {
      ...
      hpHeroDebug("lobby_wait", { gameState: gameState });
      return true;
    }
  ...
  function pauseHpHeroPresetWatchForLobby(config) {
    ...
    hpHeroDebug("lobby_pause", {
      locked: config && config.__hpHeroPresetDetectionLocked,
      lockedHero: config && config.__hpHeroPresetLockedHero
    });
    clearHpHeroDetectionRefs();
    clearHpHeroGameTimeCache();
    if (config) {
  ...
  function getHpHeroPresetLookupState(config) {
    ...
    if (gameTime > 0) {
      ...
      if (config.__hpHeroPresetDetectionLocked) {
        hpHeroDebug("lookup_locked", {
          hero: config.__hpHeroPresetLockedHero,
          gameTime: gameTime,
          lockAt: getHpHeroPresetLockAfterGameTime(config)
        });
        return { run: false, delay: HP_HERO_LOBBY_POLL_SEC };
      }
  ...
  function detectHpLocalHero() {
    var heroId = hpHeroClassOn(resolveHpHeroProgressPanel());
    if (heroId !== _hpHeroDebugLastDetectedHero) {
      _hpHeroDebugLastDetectedHero = heroId;
      hpHeroDebug("detect", {
        hero: heroId || "none",
        progress: hpHeroPanelId(_hpHeroProgress),
        crosshair: hpHeroPanelId(_hpHeroCrosshair),
        gameTime: readHpHeroGameTimeSec(),
        gameState: readHpHeroGameState()
      });
    }
    return heroId;
  }
  ...
  function setHpHeroDetectionMode(config, mode) {
    ...
    if (nextMode === HP_HERO_DETECTION_AUTO) {
      openHpHeroPresetDetectionWindow(config, "mode_auto");
      invalidateHpHeroPresetApplyCache(config);
  ...
  function selectBakedPresetForHero(modConfig, allowUnknownFallback, allowHeroMatch) {
    ...
    hpHeroDebug("select", {
      ...
      mode: normalizeHpHeroDetectionMode(modConfig)
    });
  ...
  function resetHpHeroPresetDetectionLock(config) {
    if (!config) return;
    if (config.__hpHeroPresetDetectionLocked || config.__hpHeroPresetLockedHero || config.__hpHeroPresetLockAfterGameTime) {
      hpHeroDebug("lock_reset", {
        locked: config.__hpHeroPresetDetectionLocked,
        lockedHero: config.__hpHeroPresetLockedHero,
        lockAt: config.__hpHeroPresetLockAfterGameTime || 0
      });
    }
  ...
  function openHpHeroPresetDetectionWindow(config, reason) {
    if (!config) return;
    resetHpHeroPresetDetectionLock(config);
    var gameTime = readHpHeroGameTimeSec();
    ...
    hpHeroDebug("lock_window", {
      reason: reason || "",
      gameTime: gameTime,
      lockAt: config.__hpHeroPresetLockAfterGameTime
    });
  }

  function lockHpHeroPresetDetectionIfReady(config, heroId) {
    if (!config || !heroId || config.__hpHeroPresetDetectionLocked) return false;
    var gameTime = readHpHeroGameTimeSec();
    var lockAt = getHpHeroPresetLockAfterGameTime(config);
    if (gameTime < lockAt) return false;
    config.__hpHeroPresetDetectionLocked = true;
    config.__hpHeroPresetLockedHero = String(heroId || "");
    hpHeroDebug("lock_set", {
      hero: config.__hpHeroPresetLockedHero,
      gameTime: gameTime,
      lockAt: lockAt
    });
  ...
  function applyHpColorsBakedPresetValues(config, values, presetKey, heroId) {
    ...
    if (!hasValues) {
      hpHeroDebug("apply_no_values", { key: presetKey, hero: heroId });
      return false;
    }
    ...
    if (appliedKey &&
      ...
      hpHeroDebug("apply_skip_same", {
        key: appliedKey,
        hero: appliedHero || "none",
        locked: config.__hpHeroPresetDetectionLocked,
        lockedHero: config.__hpHeroPresetLockedHero
      });
    ...
    hpHeroDebug("apply", {
      key: appliedKey,
      hero: appliedHero || "none",
      prevKey: config.__hpLastAppliedHeroPresetKey,
      prevHero: config.__hpLastAppliedHeroPresetHero,
      selectedKey: config.__anitaSelectedPresetKey,
      teamColors: values && values.hp_team_colors,
      high: values && values.hp_color_high
    });
  ...
  function refreshHpHeroPresetSelection(config) {
    ...
    if (!hpHeroAutoDetectionEnabled(config)) {
      hpHeroDebug("refresh_skip_auto_off", { mode: normalizeHpHeroDetectionMode(config) });
      return false;
    }
    if (config.__hpHeroPresetDetectionLocked) {
      hpHeroDebug("refresh_skip_locked", {
        hero: config.__hpHeroPresetLockedHero,
        key: config.__hpLastAppliedHeroPresetKey,
        mode: normalizeHpHeroDetectionMode(config)
      });
      return false;
    }
    ...
    if (detectedHero !== previousHero) {
      hpHeroDebug("hero_changed_invalidate", {
        from: previousHero || "none",
        to: detectedHero || "none",
        lastKey: config.__hpLastAppliedHeroPresetKey,
        lastHero: config.__hpLastAppliedHeroPresetHero
      });
      invalidateHpHeroPresetApplyCache(config);
      config.__hpLastDetectedHeroPresetHero = detectedHero;
    }
    hpHeroDebug("refresh", {
      previousHero: previousHero || "none",
      detectedHero: detectedHero || "none",
      reason: selection ? selection.reason : "none",
      preset: selection && selection.preset ? hpHeroDebugPreset(selection.preset) : "none",
      hasScoped: selection && selection.hasScopedPreset,
      watchStarted: config.__hpHeroPresetWatchStarted,
      emptyTicks: config.__hpHeroPresetEmptyTicks || 0,
      lastKey: config.__hpLastAppliedHeroPresetKey,
      lastHero: config.__hpLastAppliedHeroPresetHero
    });
    ...
    if (selection && selection.hasScopedPreset) {
      hpHeroDebug("refresh_wait_scoped", {
        detectedHero: detectedHero || "none",
  ...
  function startHpHeroPresetWatch(config) {
    if (!config) {
      hpHeroDebug("watch_start_skip", { reason: "no_config" });
      return;
    }
    if (config.__hpHeroPresetWatchStarted) {
      hpHeroDebug("watch_start_skip", { reason: "already_started" });
      return;
    }
    if (!config.__hpHeroPresetHasScopedPreset && !hasHpSelectedScopedPreset(config)) {
      hpHeroDebug("watch_start_skip", { reason: "no_scoped" });
      return;
    }
    config.__hpHeroPresetHasScopedPreset = true;
    ...
    config.__hpHeroPresetEmptyTicks = 0;
    hpHeroDebug("watch_start", {
      hasScoped: config.__hpHeroPresetHasScopedPreset,
      selectedKey: config.__anitaSelectedPresetKey,
      lastKey: config.__hpLastAppliedHeroPresetKey,
      lastHero: config.__hpLastAppliedHeroPresetHero
    });
    ...
    function tick() {
      ...
      var shouldLogTick = config.__hpHeroPresetWatchTicks <= 5 ||
        config.__hpHeroPresetWatchTicks % 10 === 0 ||
        !lookupState.run ||
        config.__hpHeroDebugLastLocked !== config.__hpHeroPresetDetectionLocked ||
        config.__hpHeroDebugLastLockedHero !== config.__hpHeroPresetLockedHero ||
        config.__hpHeroDebugLastRun !== lookupState.run;
      if (shouldLogTick) {
        hpHeroDebug("watch_tick", {
          tick: config.__hpHeroPresetWatchTicks,
          run: lookupState.run,
          delay: lookupState.delay,
          locked: config.__hpHeroPresetDetectionLocked,
          lockedHero: config.__hpHeroPresetLockedHero,
          lastDetected: config.__hpLastDetectedHeroPresetHero || "none"
        });
      }
      config.__hpHeroDebugLastLocked = config.__hpHeroPresetDetectionLocked;
      config.__hpHeroDebugLastLockedHero = config.__hpHeroPresetLockedHero;
      ...
      if (!config.__hpHeroPresetHasScopedPreset) {
        hpHeroDebug("watch_stop_no_scoped", {
          tick: config.__hpHeroPresetWatchTicks,
          lastKey: config.__hpLastAppliedHeroPresetKey,
          lastHero: config.__hpLastAppliedHeroPresetHero
        });
      ...
      (function (delaySec, delayIndex) {
        $.Schedule(delaySec, function () {
          ...
          try {
            ...
            hpHeroDebug("startup_try", {
              delayIndex: delayIndex,
              delay: delaySec,
              allowHero: allowHeroMatch,
              allowUnknown: allowUnknownFallback,
              lookupRun: lookupState.run,
              lookupDelay: lookupState.delay
            });
            var selection = selectBakedPresetForHero(config, allowUnknownFallback, allowHeroMatch);
            if (!selection || !selection.preset) {
              hpHeroDebug("startup_none", {
                delayIndex: delayIndex,
                reason: selection ? selection.reason : "missing",
                hasScoped: selection && selection.hasScopedPreset
              });
            ...
            if (applyHpColorsBakedPresetValues(config, selection.preset.values || {}, selection.preset.id || selection.preset.name, selection.heroId)) {
              hpHeroDebug("startup_apply", {
                delayIndex: delayIndex,
                reason: selection.reason,
                preset: hpHeroDebugPreset(selection.preset),
                hero: selection.heroId || "none"
              });
  ...
  const AnitaRenderer = {
    ...
    presetRowMatchesKey: function (row, key) {
      var text = String(key || "");
      if (!row || !text) return false;
      return String(row.key || "") === text ||
        String(row.id || "") === text ||
        this.getPresetPriorityIdentity(row) === text ||
        (row.id && ("id:" + String(row.id)) === text);
    },

    getAppliedPresetRow: function (config, rows) {
      var key = String((config && config.__hpLastAppliedHeroPresetKey) || "");
      if (!key) key = String((config && config.__anitaSelectedPresetKey) || "");
      if (!key || !Array.isArray(rows)) return null;
      for (var i = 0; i < rows.length; i++) {
        if (this.presetRowMatchesKey(rows[i], key)) return rows[i];
      }
      return null;
    },

    getPresetBuilderHeroSignature: function (config) {
      if (!config) return "";
      return [
        String(config.__hpLastDetectedHeroPresetHero || ""),
        String(config.__hpLastAppliedHeroPresetKey || ""),
        String(config.__hpLastAppliedHeroPresetHero || ""),
        String(config.__anitaSelectedPresetKey || ""),
        normalizeHpHeroDetectionMode(config),
        String(config.__hpHeroPresetDetectionLocked || false),
        String(config.__hpHeroPresetLockedHero || "")
      ].join("|");
    },

    isPresetBuilderOpenForConfig: function (config) {
      return !!(config &&
        this.isOpen &&
        this.activeModTitle === config.title &&
        this.isPresetBuilderCategory(config.__anitaActiveCategory));
    },

    schedulePresetBuilderHeroRefresh: function (config) {
      if (!this.isPresetBuilderOpenForConfig(config)) return;
      if (config.__anitaPresetBuilderHeroRefreshQueued) return;
      config.__anitaPresetBuilderHeroRefreshQueued = true;
      $.Schedule(1.0, function () {
        config.__anitaPresetBuilderHeroRefreshQueued = false;
        if (!AnitaRenderer.isPresetBuilderOpenForConfig(config)) return;
        var before = AnitaRenderer.getPresetBuilderHeroSignature(config);
        refreshHpHeroPresetSelection(config);
        var after = AnitaRenderer.getPresetBuilderHeroSignature(config);
        if (after !== before) {
          hpHeroDebug("builder_ui_refresh", { before: before, after: after });
          AnitaRenderer.renderModSettings(config);
          return;
        }
        AnitaRenderer.schedulePresetBuilderHeroRefresh(config);
      });
    },
    ...
    renderPresetBuilderPanel: function (parent, config) {
      ...
      if (this.isPresetBuilderOpenForConfig(config)) {
        var builderBefore = this.getPresetBuilderHeroSignature(config);
        refreshHpHeroPresetSelection(config);
        var builderAfter = this.getPresetBuilderHeroSignature(config);
        hpHeroDebug("builder_render_refresh", { before: builderBefore, after: builderAfter });
      }
      ...
      var appliedRow = this.getAppliedPresetRow(config, rows);
      var detectedHeroForBuilder = String(config.__hpLastDetectedHeroPresetHero || "");
      var appliedHeroForBuilder = String(config.__hpLastAppliedHeroPresetHero || "");
      ...
      status.text = getSelectedStatusText();
      ...
      function getSelectedStatusText() {
        var active = appliedRow || getSelectedRow();
        var heroName = detectedHeroForBuilder
          ? hpHeroDisplayName(detectedHeroForBuilder)
          : (appliedHeroForBuilder ? hpHeroDisplayName(appliedHeroForBuilder) : "Detecting hero");
        return active ? ("Hero: " + heroName + " • Using: " + active.name) : ("Hero: " + heroName + " • No preset selected");
      }
      ...
      for (var r = 0; r < rows.length; r++) {
        var row = rows[r];
        var rowIsApplied = AnitaRenderer.presetRowMatchesKey(row, config.__hpLastAppliedHeroPresetKey || config.__anitaSelectedPresetKey);
        var rowMatchesDetectedHero = !!(detectedHeroForBuilder && presetTargetsHero(row, detectedHeroForBuilder));
        var rowIsGlobalFallback = !!(rowIsApplied && presetIsGlobal(row));
        ...
        rowPanel.SetHasClass("Applied", rowIsApplied);
        rowPanel.SetHasClass("HeroMatch", rowMatchesDetectedHero);
        rowPanel.SetHasClass("HeroFallback", rowIsGlobalFallback);
        ...
        var metaText = row.category + " - " + row.status;
        if (rowIsApplied) metaText += " • IN USE";
        else if (rowMatchesDetectedHero) metaText += " • HERO MATCH";
        else if (presetIsGlobal(row)) metaText += " • ALL HEROES";
      ...
      config.__anitaPresetBuilderHeroUiLastSig = this.getPresetBuilderHeroSignature(config);
      this.schedulePresetBuilderHeroRefresh(config);
    },
    ...
    renderModSettings: function (config) {
      ...
      if (String(config.title || "") === "HP Colors") {
        var donateBtn = $.CreatePanel("Button", container, "AnitaDonateBtn");
