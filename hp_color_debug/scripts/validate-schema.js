...
function main() {
  ...
  for (const heroDebugMarker of [
    ...
    'builder_ui_refresh',
    'builder_render_refresh',
    'presetRowMatchesKey: function (row, key)'
  ]) {
    if (!uiCore.includes(heroDebugMarker)) {
      errors.push(`hp_color_debug missing hero debug marker: ${heroDebugMarker}`);
    }
  }
  for (const presetHighlightStyleMarker of [
    '.AnitaPresetRow.Applied',
    '.AnitaPresetRow.HeroMatch',
    '.AnitaPresetRow.Applied.HeroMatch',
    '.AnitaPresetRow.Applied.HeroFallback'
  ]) {
    if (!uiStyle.includes(presetHighlightStyleMarker)) {
      errors.push(`anita_ui.css missing preset active/hero highlight marker: ${presetHighlightStyleMarker}`);
    }
  }
  ...
  // 12. Debug hp_color base_hud must expose the builder insertion point so pak97 can carry pak96 preset store.
  const baseHud = fs.readFileSync(path.join(ROOT, 'hp_color_debug', 'panorama', 'layout', 'base_hud.xml'), 'utf-8');
  if (!baseHud.includes('id="AnitaUI_Anchor"')) {
    errors.push('base_hud.xml missing AnitaUI_Anchor for HPColorsPresetStore injection');
  }
  const buildScript = fs.readFileSync(path.join(ROOT, 'build_hp_color_debug.ps1'), 'utf-8');
  if (!fs.existsSync(path.join(ROOT, 'hp_color_debug', 'scripts', 'validate-hero-selector.js'))) {
    errors.push('hp_color_debug/scripts/validate-hero-selector.js missing');
  }
