...
- `team_neutral` wins classification before `enemy` or `friend`; `team1`/`team2` never imply relation. Building, boss, and creature facts are collected in the existing ancestry pass. A ghoul is a non-player creature that is neither a building nor a boss. Unknown teams retain the configured high color.
- The custom renderer writes fill/healing/delta and ultimate-icon `washColor`, bullet-shield `backgroundColor`, shared healthbar/ultimate-background opacity, container width/height/transform, stock pip-label visibility, rewrite-owned level visibility/tier classes, namespaced pulse classes/duration, and rewrite-owned kill-marker visibility/position/width/color. It never writes engine-owned fill, feedback, shield widths, pip or level text, icon image, or icon visibility. The kill marker is restricted to visible enemy panels with the stock `player` class; allies, neutrals, non-player units, buildings, sentries, bosses, and boss barracks always clear it. Bypass and exclusions clear rewrite-owned state so stock CSS resumes.
- Hidden enemy and ally bar containers remain rendered at `opacity: 0.01` so the engine continues updating their widths; showing them writes `opacity: 1`. Ghoul opacity applies one cached value to `UnitHealthbarContainer` and `unit_info_bg` independently of ghoul color exclusion, and its zero setting also resolves to `0.01`. Never use `visibility: collapse` or zero opacity for this control.
- The owned HP counter uses the legacy non-displacing geometry: `WindowRoot` is `100%` wide and `fit-children` high; `UnitStatus` is a bottom-aligned, centered `fit-children` down-flow; and `InfoHealthContainer` is a bottom-aligned `fit-children` × `300px` right-flow. A narrow `1px` × `399px` in-flow extent immediately before `InfoHealthContainer` enlarges the world-panel render surface upward without changing horizontal flow or the bottom-anchored healthbar position. `hp_counter_anchor` is an ignored-flow `100%` × `100%` sibling of `InfoHealthContainer` directly under `UnitStatus`; it contains a centered right-flow row with current and maximum labels. Runtime gives both labels `height: 100%`, writes the requested font size, and translates the anchor once. Team Color Max HP changes only the maximum label and falls back to the active text color when team identity is unknown.
...
4. Exercise fixed/gradient enemy colors, thresholds, visibility, width, height, healing, damage-delta, and shield-indicator colors.
5. Enable ally customization and require only friend bar, healing, damage-delta, and shield-indicator colors to change.
6. Toggle structure, objective, and ghoul color exclusions independently. On a creature-class Graves ghoul, enable custom opacity and test 0%, a middle value, and 100%; require the healthbar and ultimate-ready background to retain the same opacity when ghoul colors are excluded. Require boss-tier creatures not to inherit ghoul opacity. Disable custom ghoul opacity and require its slider and number entry to become disabled.
