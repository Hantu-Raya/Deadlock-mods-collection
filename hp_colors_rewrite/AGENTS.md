...
`hp_colors_menu.js` owns one canonical global base, ordered session-scoped snapshot rows, and a baked-before-user session preset repository. A deterministic resolver chooses the first matching Selected Heroes row, then the first All Heroes row, then the global base. Off preset application replaces the base; All/Selected application preserves that fallback and replaces the Current scope. Only the resolved effective snapshot is retained in the existing absolute-root config attribute and published through `ClientUI_FireOutput`; a separate menu-only root attribute retains the base, scope rows, user presets, and pending preset ID. Effective-value equality gates revision increments and dispatch. While effective customization is enabled, the owner replays the unchanged cached snapshot at 1-second hot, 3-second warm, then 8-second idle intervals so late contexts receive it. `healthbar_probe.js` keeps one local snapshot and ignores identical replays before parsing or rendering. There is no overlay-to-menu request path, full-tree refresh, or second settings authority.

Hero identity is transient menu-owned metadata, not part of the healthbar settings snapshot. Auto detection reads only the generated `CitadelHudTopBarPlayer.LocalPlayer` card under `#TopBar`, maps the exact `.HeroName` retail text to a stable `hero_*` key, and requires two matching active-match samples. Blank, `#`, fuzzy, and unmapped values remain unknown. Manual Override uses one explicit stable key; Off clears effective identity and skips local-card scans. Identity/lifecycle state never enters `DEFAULTS`, HPCR2, Undo, the root snapshot publication, or unit-status contexts. Scope rows and preset records validate and deduplicate stable hero keys, normalize empty Selected rows to Off, and remain session-only. A Selected preset waits without live mutation until exact identity resolves; match applies once and mismatch rejects. On later exact identity transitions, the first matching saved Selected record in session creation order replaces any stale Current row for that hero; otherwise the first saved All Heroes record restores the shared fallback. Leaving an active Selected scope without either automatically applies the baked `baked_default` record. Manual mutation cancels and suppresses a pending record for that resolving transition.
...
14. On the Overview / Hero page, verify lobby/pregame, active-match, hero-change, post-match, and next-match transitions. Auto must show a stable `hero_*` key only after settling; blank or placeholder labels must stay unknown. Manual Override must display the selected hero, Off must clear effective identity, and replaced topbar cards/clocks must recover without stale hero leakage.
15. Choose Global, All Heroes, and Selected Heroes save targets; search and multi-select stable heroes; remove the final selected hero; and require deterministic Selected → All → global fallback. Saving must automatically capture the latest editor values without a separate Capture Current action, and saving alone must not repaint or publish.
16. Save distinct Global, All Heroes, and Selected Heroes session presets; require baked records before session records and stable creation order. Require Selected → All Heroes → Rewrite Default automatic routing on exact settled hero transitions, including stale Current-row replacement and no revision/dispatch for byte-identical effective values. Without an All Heroes record, changing away from Shiv must apply the baked Rewrite Default automatically. Restart Deadlock and require session user presets to be absent.
17. Exit so `console.log` flushes; require config/role/data/identity transition logs with no rewrite exceptions.
