...
17. Save distinct All Heroes and Selected Heroes session presets; require baked records before session records and stable creation order. Install a web-builder preset VPK, restart Deadlock, and require its selected `HPCRP1` record to become Current and affect live bars before any game-mode, area, or hero lifecycle transition. Change a toggle and slider; require Current and live bars to update while the hidden base and source preset record remain unchanged until **Save & Apply**. A web-builder seed containing an All Heroes user preset must hide baked Rewrite Default while retaining its canonical fallback. Legacy user Global records must normalize to All Heroes without applying or replacing the hidden base. Require Selected → All Heroes → Rewrite Default automatic routing on exact settled hero transitions, including stale Current-row replacement and no revision/dispatch for byte-identical effective values. Without an All Heroes record, changing away from Shiv must apply baked Rewrite Default automatically. Restart Deadlock and require runtime-created session presets to be absent while the packaged builder selection applies again.
