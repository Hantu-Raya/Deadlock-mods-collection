...
- Bar-derived or custom low/mid/high text colors. Bar Color inherits the enemy bar's Fixed/Gradient mode and shared thresholds. Custom enables its own Fixed/Gradient choice. The one always-editable shared threshold pair lives under Health Info → HP Text and also drives enemy and ally bars; Enemy and Ally do not repeat it. The white label is tinted through `washColor`, matching the bar and legacy rendering path instead of assigning a darker flat `color`.
