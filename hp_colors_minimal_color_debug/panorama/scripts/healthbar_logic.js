...
(function () {
  ...
  var COLOR_DEBUG_ENABLED = true;
  var COLOR_DEBUG_MAX_LINES = 160;
  var colorDebugLines = 0;
  var lastColorDebugSig = "";
  ...
  var lPI = -1;
  var lTB = null;
  var firstEnemyPaintReported = false;
  ...
  function markPresetApplied(keyCount) {
    presetApplied = true;
    presetGeneration++;
    settingsDirty = true;
    allySettingsDirty = true;
  ...
  try {
    $.RegisterForUnhandledEvent(EVENT_CHANNEL, function (payload) {
      if (typeof payload === "string" && payload.indexOf(SNAPSHOT_MAGIC) === -1)
        return;
      try {
        if (
          presetApplied &&
  ...
  function getGradientTextColor(hp, low, high) {
    if (cfg.hp_text_color_mode) {
      if (hp <= low) return cfg.hp_text_color_low;
      if (hp <= high) {
        return ipHex(cfg.hp_text_color_low, cfg.hp_text_color_mid, (hp - low) / dc.denomMid);
      }
      return ipHex(cfg.hp_text_color_mid, cfg.hp_text_color_high, (hp - high) / dc.denomHigh);
    }
    if (hp <= low) return cfg.hp_color_low;
    if (hp <= high) {
      return ipHex(cfg.hp_color_low, cfg.hp_color_mid, (hp - low) / dc.denomMid);
    }
  ...
  function colorDebugPanel(panel) {
    ...
    while (cur && depth < 6) {
      ...
      try {
        if (cur.BHasClass) {
          if (cur.BHasClass(CLASS_ENEMY)) cls += ".enemy";
          if (cur.BHasClass(CLASS_FRIEND)) cls += ".friend";
          if (cur.BHasClass(CLASS_TEAM1)) cls += ".team1";
          if (cur.BHasClass(CLASS_TEAM2)) cls += ".team2";
          if (cur.BHasClass(CLASS_TEAM_NEUTRAL)) cls += ".team_neutral";
          if (cur.BHasClass("building")) cls += ".building";
          if (cur.BHasClass("guardian")) cls += ".guardian";
        }
      } catch (eCls) {}
      var width = 0;
      try {
        width = cur.actuallayoutwidth | 0;
      } catch (eWidth) {}
      out += (out ? " < " : "") + (id || "?") + cls + "[" + width + "]";
      try {
  ...
  function colorDebugStyle(panel) {
    if (!panel || !panel.style) return "";
    try {
      return String(panel.style.washColor || "");
    } catch (e) {}
    return "";
  }

  function colorDebug(stage, detail) {
    if (!COLOR_DEBUG_ENABLED || colorDebugLines >= COLOR_DEBUG_MAX_LINES) return;
    var sig = String(stage || "") + "|" + String(detail || "");
    if (sig === lastColorDebugSig) return;
    lastColorDebugSig = sig;
    colorDebugLines++;
    try {
      $.Msg("[HP_COLOR_DEBUG] " + sig);
    } catch (e) {}
  }
  ...
  function resolveRedBar(mode) {
    ...
    if (vPanel(us)) {
      ...
      if (redBarCandidateMatchesMode(nested, mode)) {
        ...
        redBarResolverMode = mode;
        redBarResolverAt = now;
        redBarResolverSource = "cached_unit_status";
        return nested;
      }
    }
    redBarResolverPanel = null;
  ...
  function sBC(c) {
    ...
    lColRaw = c;
    if (lCol !== next && rb) {
      var beforeColor = colorDebugStyle(rb);
      colorDebug(
        "bar-write",
        "raw=" +
          String(c) +
  ...
  function getDefaultBarColor(teamId, flags) {
    if ((flags & 4) && ((flags & 8) || (knownFriendlyTeamId && teamId === knownFriendlyTeamId))) return WHITE_WASH;
  ...
  function resetAllyBarColor(panel, flags) {
    ...
    var color = getDefaultBarColor(0, flags | 0);
    try {
    ...
    try {
      ...
      if (hp <= low) {
        ...
        if (hp <= high) {
          cl =
            cfg.hp_mode === 1
              ? ipHex(cfg.hp_color_low, cfg.hp_color_mid, (hp - low) / dc.denomMid)
              : cfg.hp_color_mid;
        } else {
          cl =
            cfg.hp_mode === 1
      ...
      colorGeneration = panelGeneration;
      if (!firstEnemyPaintReported && panelBornAt) {
        firstEnemyPaintReported = true;
      }
