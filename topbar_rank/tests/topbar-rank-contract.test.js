...
if (ALERT_EDITION) {
  ...
  assert.doesNotMatch(source, /FindChildTraverse\(["']GameTime["']\)|MISSING_WINDOW|MISSING_TOAST/, 'rank-only runtime has no clock polling for missing players');
  assert.doesNotMatch(style, /HeroImageArea[\s\S]{0,300}wash-color/, 'rank-only stylesheet has no portrait darkening rule');
}

console.log(ALERT_EDITION ? 'alert TopBarPlus rank contract tests passed' : 'rank-only TopBarPlus rank contract tests passed');
