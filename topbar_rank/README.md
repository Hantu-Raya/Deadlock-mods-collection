...
This source tree packages 22 assets: all 16 V40D assets; the profile-card, context-menu, Escape-menu, and player-list rank layouts; one ES5 Barebones runtime; and one Barebones style. The topbar-root and topbar-player layouts are merged replacements within that set, not added overlays. Native `GameTime` and every TopBarPlus script include remain intact.

The Barebones runtime owns rank behavior: direct account-ID witnesses, duplicate-name/account safety, bounded Escape probing, six/twelve verified-account caching, native Escape behavior, and ranks on profile, context menu, topbar, team average, and player list. It remains separate from standalone `showrank_barebones`, which is a different mod and release path.

## Alert-only behavior

This edition also keeps the alert features: missing labels, portrait darkening, clock polling, the notification root, and hero-icon announcements. Those features are intentionally absent from `../topbar_rank_no_missing`.
...
Use `-KeepStaging` to retain staging files. Use `-Install` only to place the result at:
...
Source and build validation can verify source contracts, staged content, and the artifact. They are not live in-game proof. Verify rank rendering, Escape behavior, TopBarPlus UI, and alert behavior in Deadlock after installation.
