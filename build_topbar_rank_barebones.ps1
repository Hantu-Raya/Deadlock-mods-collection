...
$vpkOutput = Join-Path $root 'topbar_rank_barebones_dir.vpk'
...
function Assert-TopbarRankAssetSet {
    param(
    ...
    foreach ($asset in $ExpectedAssets) {
        if (-not $expected.Add($asset)) { throw "Topbar Rank required asset is duplicated: $asset" }
    }
...
function Get-TopbarRankAssetPaths {
    ...
    if (-not (Test-Path -LiteralPath $panoramaRoot)) {
        throw "Topbar Rank panorama folder not found: $panoramaRoot"
    }
    return @(
...
function Get-TopbarRankPackedAssetPaths {
...
function Assert-TopbarRankProject {
    if (-not (Test-Path -LiteralPath $projectRoot)) {
        throw "Topbar Rank source folder not found: $projectRoot"
    }
    Assert-TopbarRankAssetSet -Actual (Get-TopbarRankAssetPaths -RootPath $projectRoot) -ExpectedAssets $requiredSourceAssets -Label 'Topbar Rank source package'
}
...
function Remove-TopbarRankInstallTemporary {
    ...
    if ((Split-Path -Leaf $Path) -ne 'pak89_dir.topbar-rank-barebones.tmp.vpk') {
        throw "Refusing to remove unexpected temporary install file: $Path"
    }
    Remove-Item -LiteralPath $Path -Force
}

function Install-TopbarRankVpk {
    param([Parameter(Mandatory = $true)][string]$SourceVpk)

    $destination = Join-Path $AddonsPath 'pak89_dir.vpk'
    $temporary = Join-Path $AddonsPath 'pak89_dir.topbar-rank-barebones.tmp.vpk'
    $replaceBackup = Join-Path $AddonsPath 'pak89_dir.topbar-rank-barebones.replace-backup.tmp.vpk'

    try {
        Assert-DeadlockClosed
        if (-not (Test-Path -LiteralPath $AddonsPath)) { throw "Deadlock addons folder not found: $AddonsPath" }
        if (Test-Path -LiteralPath $replaceBackup) { throw "A preserved Topbar Rank replacement backup requires recovery or removal: $replaceBackup" }

        Copy-Item -LiteralPath $SourceVpk -Destination $temporary -Force
        $sourceHash = Get-TopbarRankSha256 -Path $SourceVpk
        $temporaryHash = Get-TopbarRankSha256 -Path $temporary
        if (-not $sourceHash.Equals($temporaryHash, [System.StringComparison]::OrdinalIgnoreCase)) {
            throw "Installed Topbar Rank VPK hash does not match build artifact: $temporary"
        }

        $temporaryTree = Get-PackedVpkTree -VpkEditCli $vpkEditCli -VpkPath $temporary -Source2ViewerPath $source2Viewer
        Assert-TopbarRankAssetSet -Actual (Get-TopbarRankPackedAssetPaths -Tree $temporaryTree) -ExpectedAssets $requiredCompiledAssets -Label 'Temporary Topbar Rank VPK'
        ...
        if (-not $sourceHash.Equals((Get-TopbarRankSha256 -Path $destination), [System.StringComparison]::OrdinalIgnoreCase)) {
            throw "Installed Topbar Rank VPK hash does not match build artifact: $destination"
        }
    } catch {
        Remove-TopbarRankInstallTemporary -Path $temporary -AddonsRoot $AddonsPath
        throw
    }
}
...
if (Test-Path -LiteralPath $vpkOutput) {
    ...
    if ((Split-Path -Leaf (Resolve-Path -LiteralPath $vpkOutput).Path) -ne 'topbar_rank_barebones_dir.vpk') {
        throw "Refusing to remove unexpected Topbar Rank VPK: $vpkOutput"
    }
...
try {
    ...
    Assert-TopbarRankAssetSet -Actual (Get-TopbarRankAssetPaths -RootPath $stageCompiled) -ExpectedAssets $requiredCompiledAssets -Label 'Compiled Topbar Rank output'
    ...
    Assert-TopbarRankAssetSet -Actual (Get-TopbarRankPackedAssetPaths -Tree $packedTree) -ExpectedAssets $requiredCompiledAssets -Label 'Packed Topbar Rank VPK'

    if ($Install) { Install-TopbarRankVpk -SourceVpk $vpkOutput }
} finally {
    if (-not $KeepStaging) {
        Remove-TreeUnderRoot -Path $buildRoot -RootPath $root -ExpectedLeaf '_topbar_rank_barebones_build'
    }
