(() => {
  ...
  const NEUTRAL_RING_SHOW_MS = 60000;   // ring appears in last 60s
  const NEUTRAL_RING_FADE_MS = 30000;   // ring fades in between 60s→30s remaining
  const NEUTRAL_RING_SEMI_OPACITY = 0.85; // fully visible under 30s
  ...
  function loop() {
    ...
    // Scan + render in a single pass
    const snapshot = collectMinimapSnapshot(nowMs);
    if (snapshot) {
      scanNeutralRespawnState(snapshot, nowMs, gameNow);
    }

    if (_neutralRespawnState.size > 0 || scoreboardOpen || scoreboardJustOpened) {
      renderNeutralRespawnTimers(nowMs, gameNow, scoreboardOpen);
    }

    // Adaptive loop rate:
    // - 0.25s when scoreboard is held open (keep opacity in sync)
    ...
    if (scoreboardOpen) {
      interval = 0.25;
    } else if (_neutralRespawnState.size === 0 && gameNow > 60) {
      interval = 2.0;
    } else {
