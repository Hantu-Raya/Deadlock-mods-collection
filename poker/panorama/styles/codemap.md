...
- **Floating window composition:** `.PokerFloatingWindow` is the shared base for Poker's five windows and Bluff Deck's `#BluffDeckWindow` plus noninteractive `#BluffDeckHistoryWindow`. Each window uses absolute-ish Panorama alignment/margins rather than normal document flow:
  ...
  - Bluff uses a content-height, 42%-wide main window at 21%/12% with a 14% × 56% archive at 22%/12%, leaving a 1% gap. The main window contains a flat 300px circular felt, cardinal outer-edge seats-as-roster, reduced cards, a centered action deck, and lifecycle footer; the empty lobby hand collapses instead of reserving action space.
...
- `#PokerTableSurface` is a mint-bordered dark panel; `#PokerTableFelt` remains Poker's oval felt while `.BluffDeckFelt` is a flat compact 300px circle. Bluff's seat plane is a centered 300px overlay: the one-player waiting `SeatBottom` and the four-player top/right/bottom/left classes are positioned fully outside the felt, while the larger table stage contains them without overlapping adjacent controls. Both use `overflow: noclip` for the edge projection.
