...
- `#PokerTableSurface` is a mint-bordered dark panel; `#PokerTableFelt` remains Poker's oval felt while `.BluffDeckFelt` is a flat compact 300px circle. Bluff's seat plane is a centered 300px overlay and remaps its four-player classes to top/right/bottom/left anchors fully outside the felt; the larger table stage contains those outer seats without overlapping adjacent controls. Both use `overflow: noclip` for the edge projection.
