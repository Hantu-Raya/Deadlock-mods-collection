...
- **Floating window composition:** `.PokerFloatingWindow` is the shared base for Poker's five windows and Bluff Deck's `#BluffDeckWindow` plus noninteractive `#BluffDeckHistoryWindow`. Each window uses absolute-ish Panorama alignment/margins rather than normal document flow:
  ...
  - Bluff uses a 69% × 88% dominant main window at 3%/6% with a 23% × 88% archive at 3%/6%, leaving an intentional 2% gap; the main window contains felt, seats-as-roster, cards, action deck, and lifecycle footer.
...
- `#PokerTableSurface` is a mint-bordered dark panel; `#PokerTableFelt` and `.BluffDeckFelt` are supported green felt surfaces with `overflow: noclip` so table seats can hang off the edge.
