...
- **Floating window composition:** `.PokerFloatingWindow` is the shared base for Poker's five windows and Bluff Deck's `#BluffDeckWindow` plus noninteractive `#BluffDeckHistoryWindow`. Each window uses absolute-ish Panorama alignment/margins rather than normal document flow:
  ...
  - Bluff uses a content-height, 42%-wide main window at 21%/12% with a 14% × 56% archive at 22%/12%, leaving a 1% gap. The main window contains a flat 300px circular felt inside a 560px × 460px in-bounds seat plane, reduced cards, a centered 148px hand/action deck, and a 48px lifecycle footer; the empty lobby hand collapses instead of reserving action space.
...
- `#PokerTableSurface` is a mint-bordered dark panel; `#PokerTableFelt` remains Poker's oval felt while `.BluffDeckFelt` is a flat compact 300px circle. Bluff's 560px × 460px seat plane surrounds that felt inside the table stage, keeping the one-player waiting `SeatBottom` and four-player top/right/bottom/left classes fully visible without relying on negative out-of-bounds margins. Both the seat plane and table stage retain `overflow: noclip` for cards and turn arrows.
