...
Committed games persist as `{ version, sourceChatSeq, game }` in `BluffDeckMatchState`. Hydration is accepted only after invariants plus retained matching start/high-water chat evidence; otherwise it fails closed. The ESC menu opens a two-row `TABLE GAMES` picker. Bluff Deck projects through a pure `BluffDeckViewModel` into exactly two compact windows: a content-height, 42%-wide main surface centered on a 300px circular felt with cardinal seats fully outside its edge, reduced cards, centered actions, and a collapsed empty lobby hand; and a 14%-wide noninteractive archive ledger showing the latest twelve committed turns. The renderer uses keyed log/played-card rows, preserves local card masking, and maps pending/challenge/turn/result states into semantic classes. It keeps the engine's protocol/history semantics separate from the presentation ledger and makes no secure-dealing claim.
