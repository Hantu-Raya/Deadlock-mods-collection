...
Committed games persist as `{ version, sourceChatSeq, game }` in `BluffDeckMatchState`. Hydration is accepted only after invariants plus retained matching start/high-water chat evidence; otherwise it fails closed. The ESC menu opens a two-row `TABLE GAMES` picker. Bluff Deck projects through the shared table seat/card/flip/action/log presenters, preserves local card masking, and keeps its own persisted history projection. It makes no secure-dealing claim.
