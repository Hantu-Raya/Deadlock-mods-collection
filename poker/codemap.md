...
Committed games persist as `{ version, sourceChatSeq, game }` in `BluffDeckMatchState`. Hydration is accepted only after invariants plus retained matching start/high-water chat evidence; otherwise it fails closed. The ESC menu opens a two-row `TABLE GAMES` picker. Bluff Deck projects through a pure `BluffDeckViewModel` into four sibling surfaces: centered table/felt, roster/lifecycle, history/feed, and card-picker/action rail. The renderer uses keyed roster/log/played-card rows, preserves local card masking, and maps pending/challenge/turn/result states into semantic announcement and control classes. It keeps its own persisted history projection and makes no secure-dealing claim.
