...
Debug output is enabled for this debug build with `[ProfileStatsCommunity]`; it is transition-only (boot, identity changes, open/request/cache, bridge events, rejects, success/errors, timeout, restore, hero-selection signals), never scheduled-poll or full-payload logging.
## Bridge contract
