(()=>{"use strict";
  /*
   * Soul Timer v4.2 - Performance Optimized
   * drain_per_second = 0.5% of remaining + 1.6 * (1 + 0.08 * gameMinutes)
   * 
   * Optimizations: 150ms display tick, cached Date.now(), change detection in slowLoop only
   */

  // === CONSTANTS ===
  const PCT_DRAIN = 0.005, BASE_FLAT = 1.6, FLAT_GROWTH = 0.08;
  const SIM_STEP = 0.5, MAX_SIM = 600;
  const THRESH_RED = 1000, THRESH_YELLOW = 500;
  
  // Timing: Fixed display tick, separate slow tick for expensive ops
  const TICK_DISPLAY = 0.15;     // 150ms for smooth countdown
  ...
  function boot() {
    ...
    $.Msg("[SoulTimer] v4.2 - Performance Optimized\\n");
    gen++;
    lastTick = Date.now();
    displayLoop(gen);
    slowLoop(gen);
    ...
    try {
      ...
      if (UI.unsec?.IsValid?.()) {
        ...
        if (raw && raw.charCodeAt(0) !== 123) {
          ...
          } else if (souls <= 0 && pSouls > 0) {
            // Souls dropped to zero - timer will hide on next displayLoop tick
            pSouls = 0;
            drainEndTime = 0;
          }
        }
      ...
      if (UI.unsec?.IsValid?.()) {
        ...
        if (raw && raw.charCodeAt(0) !== 123) {
          ...
          if (souls > 0) {
            ...
            const gameMin = gTime() / 60;
            const drainSec = calcDrainTime(souls, gameMin);
            drainEndTime = now + drainSec * 1000;
            setColor(getColorIdx(souls));
          }
        }
      }
